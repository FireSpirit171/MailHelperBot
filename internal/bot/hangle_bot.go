...
func handleStartCommand(bot *Bot, msg *tgbotapi.Message) {
	text := `Привет! Я бот для работы с почтой Mail.ru.
...
func handleLoginCommand(bot *Bot, msg *tgbotapi.Message) {
	...
	text := fmt.Sprintf(`<b>Для авторизации выполните следующие шаги:</b>
...
<a href="%s">🔐 Авторизоваться через Mail.ru</a>

<i>Ссылка действительна в течение 10 минут</i>`, authURL)
